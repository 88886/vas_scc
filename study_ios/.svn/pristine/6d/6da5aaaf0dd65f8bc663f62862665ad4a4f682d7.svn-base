...
- (BOOL)saveChatMsg:(ChatMsgDTO *)msg{
    ...
    if (![self queryMsgIsExist:msg.msgid]) {
        ...
        [queue inDatabase:^(FMDatabase *database) {
            //插入数据
            [database executeUpdate:@"insert into zChatMsgList(zmsgid,zfromuid,ztouid,zcontent,zsendtime,zorderytime,zfiletype,zsendtype,zurl,zisread,zname,zthumbnail,zduration,zsuccess,zlocalpath,zownerid,zchattype,zprgress,ztotalsize,chatphoto,receiverread) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",msg.msgid,msg.fromuid,msg.touid,msg.content,msg.sendtime,[JZCommon stringToDate:msg.sendtime format:@"yyyy-MM-dd HH:mm:ss.SSS"],[NSString stringWithFormat:@"%ld",(long)msg.filetype],[NSString stringWithFormat:@"%ld",(long)msg.sendtype],msg.url,msg.isread,msg.name,msg.thumbnail,msg.duration,[NSString stringWithFormat:@"%ld",(long)msg.success],msg.localpath,[JZCommon getUserPhone],[NSString stringWithFormat:@"%ld",(long)msg.chatType],msg.progress,msg.totalsize,msg.chatphoto,msg.receiverread];
        }];
    }
    return result;
}

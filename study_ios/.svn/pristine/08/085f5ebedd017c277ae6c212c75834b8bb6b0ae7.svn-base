...
- (MsgSummary *)queryMsgSummaryBychatid:(NSString *)fromId{
    ...
    [queue inDatabase:^(FMDatabase *database) {
        FMResultSet *resultSet = [database executeQuery:@"SELECT * FROM zMsgSummary where zownerid=? and zchatid=? order by ztoptime,zdate",[JZCommon queryLoginUserId],fromId];
...
-(BOOL)saveMsgSummary:(MsgSummary *)msg{
    ...
    [queue inDatabase:^(FMDatabase *database) {
    //插入数据
    result=[database executeUpdate:@"insert into zMsgSummary(zchartype,zcount,zfiletype,zistop,zdate,zchatid,zcontent,ztoptime,zownerid,zstate,zisread,zmsgid,chatphoto) values (?,?,?,?,?,?,?,?,?,?,?,?,?)",[NSString stringWithFormat:@"%ld",(long)msg.chartype],[NSString stringWithFormat:@"%ld",(long)msg.count],[NSString stringWithFormat:@"%ld",(long)msg.filetype],[NSString stringWithFormat:@"%ld",(long)msg.istop],msg.date,msg.chatid,msg.content,msg.toptime,msg.ownerid,msg.state,msg.isRead,msg.msgid,msg.chatphoto];
...
- (void)dealMsgSummary:(ChatMsgDTO *)chatmsg :(NSString*)chatid{
    ...
    //查询聊天对象是否存在
    MsgSummary *msg=[self queryMsgSummaryBychatid:userid];
    msg.toptime = chatmsg.isStar;//消息来源
    if (![JZCommon isBlankString:msg.ownerid]) {
        //对外沟通助手
        if (msg.chartype==2 ) {
...
//        msg.date =[NSDate date];
    ...
    //新聊天对象
        ...
        userMessage.toptime = chatmsg.isStar;

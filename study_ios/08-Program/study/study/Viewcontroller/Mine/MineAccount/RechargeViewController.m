...
- (void)creatUI{
    ...
    UILabel *cashLabel = [[UILabel alloc] initWithFrame:CGRectMake(MinX(_bankImage), 0, widget_height(80), widget_height(80))];
    [self setLabelStatus:cashLabel WithTitle:@"金额" withFont:14 color:UIColorFromRGB(0x434343)];
    cashLabel.textAlignment = NSTextAlignmentCenter;
    [cashView addSubview:cashLabel];
    //输入金额框
    _moneyText = [[UITextField alloc] initWithFrame:CGRectMake(MinX(_bankNameLabel), 0, Main_Width - MinX(_bankNameLabel) - 30, widget_height(80))];
    ...
    //充值按钮
    UIButton *rechargeBtn = [[UIButton alloc] initWithFrame:CGRectMake((Main_Width - widget_width(580)) * 0.5, MaxY(cashView) + widget_height(50), widget_width(580), widget_height(80))];
    [self setBtnStatus:rechargeBtn WithBackColor:UIColorFromRGB(0xff6949) withTitle:@"确认充值" withTitleColor:[UIColor whiteColor] withFont:15];
...
{
    NSLog(@"确认充值,金额为%@，卡号为%@", _moneyText.text, _bankCardInfoDic[@"num"]);
}
